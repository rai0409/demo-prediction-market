...
FastAPIベースのローカルMVPです。外部予測市場の公開参考データを取得し、表示可能な市場だけをダッシュボードに出し、ローカルのデモポイントで `予想する` / `デモ参加する` 体験をシミュレーションできます。
...
The following commands are for internal checks only and are not part of the general MVP user flow.
...
The FastAPI app does not start the socket automatically. The standalone runner stores public market observations for developer diagnostics. The general MVP UI uses user-facing update labels instead of transport-level wording.
